...
unsigned char PS2X::_gamepad_shiftinout(char byte) {
  uint8_t old_sreg =
      SREG;  // *** KJE *** save away the current state of interrupts
...
void PS2X::read_gamepad(boolean motor1, byte motor2) {
  ...
  uint8_t old_sreg = SREG;  // *** KJE **** save away the current state of
                            // interrupts - *** *** KJE *** ***
...
byte PS2X::config_gamepad(uint8_t clk, uint8_t cmd, uint8_t att, uint8_t dat,
  ...
  if (PS2data[1] != 0x41 && PS2data[1] != 0x73 &&
      PS2data[1] != 0x79) {  // see if mode came back. If still anything but 41,
                             // 73 or 79, then it's not talking
#ifdef PS2X_DEBUG
